...
const afn = Config.ALINS !== false ? Config.ALINS.split(',') : [];
const afnp = Poocha-Ser.ALINAFN !== false ? Poocha-Ser.ALINAFN.split(',') : [];
...
Mybot.addCommand({pattern: 'filter ?(.*)', fromMe: true, desc: Lang.FILTER_DESC, dontAddCommandList: true}, (async (message, match) => {
...
Mybot.addCommand({pattern: 'stop ?(.*)', fromMe: true, desc: Lang.STOP_DESC, dontAddCommandList: true}, (async (message, match) => {
    match = match[1].match(/[\'\"\“](.*?)[\'\"\“]/gsm);
    if (match === null) {
...
Mybot.addCommand({on: 'text', fromMe: false }, (async (message, match) => {
    if(Config.BGMFILTER){
...
async function checkImAdmin(message, user = message.client.user.jid) {
    ...
    return sonuc.includes(true);
     ...
     Mybot.addCommand({on: 'text', fromMe: false,onlyGroup: true}, (async (message, match) => {
...
if(pattern.test(message.message)){
    ...
    filtreler.map(
     ...
     Mybot.addCommand({on: 'text', fromMe: false, onlyPm: true}, (async (message, match) => {
